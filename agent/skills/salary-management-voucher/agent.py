...
"""
工资报酬管理帐凭证生成 - Skill 执行入口（deep-agent 工具）

供 deep-agent 的 LLM 通过工具调用此 skill。

工具列表：
  - invoke_salary_management_voucher: 生成工资报酬管理帐凭证
"""
...
def _run_salary_management_voucher(
    input_file: str | Path,
    ...
    if not template_path:
        return {"success": False, "message": "凭证模板（凭证表.xlsx.xlsx）未找到，请放在 skill 目录或桌面"}

    # 账簿映射
    account_book_path = _find_file("公司主体账套号.xlsx", "*.xlsx")

    # 客户映射（管理帐用不同的映射文件）
    customer_path = _find_file("客户帐套_管理.xlsx", "*.xlsx")
...
def _find_file(name: str, _pattern: str) -> Path | None:
    """在 skill 目录和桌面搜索指定文件。"""
...
def _run_via_import(
    ...
    """通过 import 方式调用脚本函数。"""
    from salary_payment_management_voucher_generator import (
        generate_salary_payment_management_voucher_files,
    )
    ...
    try:
        ...
        out_file = output_dir / "工资发放管理帐凭证.xlsx"
        return {
            "success": True,
...
def invoke_salary_management_voucher(
    input_file: str,
    output_dir: str | None = None,
) -> str:
    """
    生成工资报酬管理帐凭证。根据工资报酬支付申请审批表自动生成金蝶云管理帐凭证。
    ...
    if not input_file:
        return json.dumps(error_response(MissingParameterError(
            "invoke_salary_management_voucher",
            "input_file",
        )), ensure_ascii=False)
    try:
        result = _run_salary_management_voucher(
...
TOOLS = [
    {
        "type": "function",
        "function": {
            "name": "invoke_salary_management_voucher",
            "description": (
                "生成工资报酬管理帐凭证。根据工资报酬支付申请审批表（金蝶云格式）自动生成管理帐凭证。"
                "按归属利润中心（广东公司 vs 其他）和内部业务类型（承揽/派遣/代理/灵工/助残）自动选择科目，生成借贷分录。"
                "广东公司承揽业务使用主营业务成本科目，其他使用应付账款科目；非广东公司使用其他应收款科目。"
